...
class Predictor(BasePredictor):
    ...
    def predict(
        ...
        # Calculate effective resolution based on input and max_resolution constraints
        if media_kind == "video":
            ...
            # Get input dimensions for resolution calculation
            h, w = tensor.shape[2], tensor.shape[3]
            # Calculate target resolution maintaining aspect ratio
            min_dim = min(h, w)
            if min_dim > 0:
                scale = resolution_val / min_dim
                target_h, target_w = int(h * scale), int(w * scale)
                # Apply max_resolution constraint if set
                if max_resolution_val > 0:
                    if target_h > max_resolution_val or target_w > max_resolution_val:
                        scale = max_resolution_val / max(h, w)
                        target_h, target_w = int(h * scale), int(w * scale)
                target_resolution = (target_h * target_w) ** 0.5
            else:
                target_resolution = (1280 * 720) ** 0.5
            # Create dynamic transform with calculated resolution
            transform = Compose(
                [
                    NaResize(resolution=target_resolution, mode="area", downsample_only=False),
                    Lambda(lambda x: torch.clamp(x, 0.0, 1.0)),
                    DivisibleCrop((16, 16)),
                    Normalize(0.5, 0.5),
                    Rearrange("t c h w -> c t h w"),
            ...
            if min_dim > 0:
                ...
                target_resolution = (2560 * 1440) ** 0.5
            # Create dynamic transform with calculated resolution
            transform = Compose(
                [
                    NaResize(resolution=target_resolution, mode="area", downsample_only=False),
